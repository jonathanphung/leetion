/**
 * super-board QA — issue #20. Loader for the REAL background.js.
 *
 * Carried forward unchanged from the issue-#11 suite except for the exposed
 * list, which now also hands back `saveToNotion` / `updatePageContent` so the
 * new `scheduleSpacedRepetitionToday` flag can be traced end to end through
 * every positional argument list it has to survive.
 *
 ...
 * click does, dispatcher included.
 */
...
export function loadBackground({ file, isoNow, fetch: fetchDouble, sync = {} }) {
  ...
  // background.js (the negative control), where localDateString/localDateInDays
  // do not exist yet — that run must produce assertion failures, not a
