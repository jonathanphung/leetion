/**
 * super-board QA — issue #20. In-memory Notion double.
 *
 * Carried forward byte-for-byte from the issue-#11 suite (which grew it from
 * the #10 one). Reused rather than re-written so the staged-switch card is
 * measured against the same Notion model the clear-review card was.
 *
...
export function createNotionMock(seed = {}) {
  ...
  async function fetchDouble(url, options = {}) {
    ...
    if (
      ...
      (!failNext.match || failNext.match(endpoint, method))
    ) {
  ...
  return {
     ...
     *
     * `match(endpoint, method)` narrows it to one kind of call. Added for
     * issue #20: the failure under test there is a *save*, and a save makes a
     * schema probe (and its retries) before it ever reaches the page PATCH, so
     * an unfiltered "fail the next 3" is eaten by the probe and the PATCH then
     * succeeds — the test would report a passing save as a failed one.
     */
    failNextWith: (status, message, times = 3, match = null) => {
      failNext = { status, message, remaining: times, match };
    },
    clearFailure: () => {
      failNext = null;
    },
